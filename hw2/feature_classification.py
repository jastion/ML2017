...
def sort_data(inputCsv):
	...
	idxArray = np.array([[idxEmployer],[idxEducation],[idxMarital],\
					[idxOccupation], [idxFamRole],[idxRace],[idxCountry]])

	finalArray = np.vstack((arrayAge,arrayWgt))
	finalArray = np.vstack((finalArray,arraySex.reshape(1,arraySex.shape[0])))
	finalArray = np.vstack((finalArray,arrayCapital.reshape(2,arrayCapital.shape[0])))
	...
	print(finalArray.shape)
	np.savetxt("a.csv", finalArray, delimiter = ",", fmt = "%s")
	return finalArray
