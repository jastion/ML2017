...
first_layer = 64*2 #64
second_layer = 128*2 #128
third_layer = (256*2) #256 #4
fourth_layer = (410*3)+18#350 
...
with open(sys.argv[1]) as trainFile:
  ...
  x_train_data = x_arr.reshape(x_arr.shape[0], 48*48).astype(np.float32)#(28709,48,48,1)
...
#odel.add(Conv2D(first_layer,(3,3) ,padding="valid", input_shape = (48,48,1)))
model.add(Dense(first_layer, activation='relu', kernel_initializer='normal', input_shape = (48*48,)))
...
model.add(Dense(second_layer, activation='relu', kernel_initializer='normal'))
model.add(BatchNormalization())
model.add(Activation("relu"))
#model.add(MaxPooling2D(2,2))
...
model.add(Dense(third_layer, activation='relu', kernel_initializer='normal'))
model.add(BatchNormalization())
model.add(Activation("relu"))
#model.add(MaxPooling2D(2,2))
...
model.add(Dense(fourth_layer, activation='relu', kernel_initializer='normal'))
model.add(BatchNormalization())
model.add(Activation("relu"))
#model.add(MaxPooling2D(2,2))
...
dir_cnt = 0
log_path = "DNN_epoch_{}".format(str(nb_epoch))
log_path += '_'
...
with open(sys.argv[2]) as testFile:
  ...
  x_test_data = test_x_arr.reshape(test_x_arr.shape[0], 48*48).astype(np.float32)#(28709,48,48,1)
